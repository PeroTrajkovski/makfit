...
function AppContent() {
        ...
        unsubProfile = onSnapshot(doc(db, 'profiles', uid), snap => {
          if (snap.exists()) {
            ...
            if (pData.isPremium === undefined) {
              updateDoc(doc(db, 'profiles', uid), { isPremium: false });
          ...
          if (!savingRef.current) setLoading(false);
          setIsAppReady(true);
  ...
  // Scroll to top before the browser paints the new view
  useLayoutEffect(() => {
    window.scrollTo(0, 0);
  }, [view]);
  ...
  if (view === 'auth') {
    return (
      <AuthView
        authMode={authMode} setAuthMode={mode => { setAuthMode(mode); setAuthError(null); setPassword(''); setConfirmPassword(''); setShowPassword(false); setShowConfirmPassword(false); setFirstName(''); setLastName(''); }}
  ...
  return (
    <>
      {typeof document !== 'undefined' && createPortal(
        <AnimatePresence>
          {activeAchievement && profile?.isPremium && (
            <motion.div
      ...
      <AnimatePresence mode="wait">
        ...
        {view === 'search' && (
          <SearchView
            key="search"
            user={user} isPremium={profile?.isPremium ?? false} selectedMealType={selectedMealType}
        ...
        {view === 'mealplan' && profile?.isPremium && (
          <AiMealPlanView
            key="mealplan"
            profile={profile}
            meals={meals}
        ...
        {view === 'challenges' && profile?.isPremium && (
          <ChallengesView
            key="challenges"
            user={user}
            profile={profile}
